...
export const CartProvider = ({children}) => {
    ...
    if(!cart){
        cart.push({itemid,count,itemprice});
            ...
            // Se agrega item en caso de ya existir
            if(isInCart({itemid}))
                {
    ...
    removeItem({itemid}); //saca un itemid del carro
    clear(); //limpia el carro completamente
}
    ...
    let sum=0;
        ...
        return (<CartContext.Provider 
                value={{cart,additem,removeItem,clear,isInCart,isInCartIndex,SumItems,total}}> {children}
